 ...
 *
 * Modes:
 *   SETUP  – AP captive portal for WiFi + pairing code provisioning
 *   RUNTIME – connect to WiFi, discover gateway, stream ADC readings
 *
 * Config is persisted in NVS (Preferences library).
 * Gateway API: POST /api/v1/ingest  (batch readings)
...
void setup() {
  ...
  // Init STA briefly to get the correct STA MAC address
  WiFi.mode(WIFI_STA);
  delay(100);
  ...
  Serial.printf("[GreenMind] Cached GW: '%s'  Code: '%s'\n",
...
void loop() {
  if (!isProvisioned) {
    // Setup mode: handle DNS + HTTP
    dnsServer.processNextRequest();
    server.handleClient();
  } else {
    // Runtime mode: stream data
    streamReadings();
  }
}
...
void saveConfig() {
  ...
  prefs.end();
  Serial.println("[GreenMind] Config saved to NVS");
}
...
void startSetupMode() {
  ...
  Serial.printf("[GreenMind] AP SSID: %s  IP: %s\n",
  ...
  server.begin();
  Serial.println("[GreenMind] Setup server ready");
}
...
void startRuntimeMode() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[GreenMind] WiFi connection failed! Rebooting...");
    delay(3000);
    ESP.restart();
  }

  Serial.printf("[GreenMind] WiFi connected  IP: %s\n",
  ...
  Serial.printf("[GreenMind] Gateway found at %s\n", gatewayIP.c_str());

  // Register sensor if pairing code exists
  if (pairingCode.length() > 0) {
    if (registerSensor()) {
      Serial.println("[GreenMind] Sensor registered successfully");
    } else {
      Serial.println("[GreenMind] Registration failed, continuing anyway");
    }
    // Clear pairing code regardless – don't retry on every boot
    pairingCode = "";
    saveConfig();
  }

  // Start a small HTTP server for DELETE factory-reset commands
  server.on("/", HTTP_DELETE, []() {
    server.send(200, "text/plain", "OK");
    delay(500);
    factoryReset();
  });
  // Health endpoint so gateway discovery works bidirectionally
  server.on("/health", HTTP_GET, []() {
    server.send(200, "application/json", "{\"status\":\"ok\"}");
  });
  server.begin();

  Serial.println("[GreenMind] Streaming started");
}
...
bool discoverGateway() {
  ...
  if (gatewayIP.length() > 0) {
    Serial.printf("[GreenMind] Trying cached gateway: %s\n", gatewayIP.c_str());
    if (checkGatewayHealth(gatewayIP)) {
      Serial.println("[GreenMind] Cached gateway OK");
      return true;
    }
    Serial.println("[GreenMind] Cached gateway unreachable, clearing");
    gatewayIP = "";
  }

  // 2) UDP broadcast discovery
  Serial.println("[GreenMind] UDP broadcast discovery...");
  WiFiUDP disc;
  disc.begin(50001);  // local port for receiving replies
    ...
    while (millis() - start < 2000) {
      ...
      if (len > 0) {
        ...
        if (msg.startsWith("GATEWAY_IP:")) {
          // Use the packet's source IP (reachable) instead of payload IP
          // (gateway may have multiple interfaces: eth0 vs wlan0)
          String sourceIP = disc.remoteIP().toString();
          String payloadIP = msg.substring(11);
          Serial.printf("[GreenMind] UDP reply: payload=%s source=%s\n",
                        payloadIP.c_str(), sourceIP.c_str());
          disc.stop();
          // Prefer source IP, fall back to payload IP
          if (checkGatewayHealth(sourceIP)) {
            gatewayIP = sourceIP;
          } else if (checkGatewayHealth(payloadIP)) {
            gatewayIP = payloadIP;
          } else {
            Serial.println("[GreenMind] UDP IPs both unreachable");
            break;
          }
          saveConfig();
          return true;
  ...
  // 3) Subnet scan fallback (with WDT feeding)
  Serial.println("[GreenMind] Subnet scan...");
  IPAddress myIP = WiFi.localIP();
  // Scan common DHCP ranges first, then remaining
  int scanOrder[] = {84}; // try known gateway IP first
  for (int host = 1; host < 255; host++) {
    ...
    WiFiClient client;
    client.setTimeout(100);  // Very short timeout
    if (client.connect(candidate, 80)) {
      ...
      if (resp.indexOf("hardware_id") >= 0) {
        ...
        gatewayIP = candidateStr;
        Serial.printf("[GreenMind] Found gateway via scan: %s\n", gatewayIP.c_str());
        saveConfig();
        return true;
      }
    }
    if (host % 10 == 0) {
      Serial.printf("[GreenMind] Scanned %d/254...\n", host);
    }
  }
...
bool registerSensor() {
  Serial.printf("[GreenMind] Registering with code: %s\n", pairingCode.c_str());
  ...
  Serial.printf("[GreenMind] Register response: %d %s\n", code, resp.c_str());
  return (code == 200 || code == 201);
}

// ── Signal Filtering ──────────────────────────
// 3-sample moving average for noise reduction (from exhibition firmware)
...
// 380 Hz sampling with timer-based precision

static const int SAMPLE_RATE = 380;
static const int SAMPLE_INTERVAL_US = 1000000 / SAMPLE_RATE;  // ~2632 µs
static const int BATCH_SIZE = 380;         // 1 second of data per POST
...
void streamReadings() {
  // Handle any incoming DELETE requests (non-blocking)
  server.handleClient();

  // Check WiFi every 5 seconds (don't block sampling loop)
  if (millis() - lastWifiCheck > 5000) {
    lastWifiCheck = millis();
    if (WiFi.status() != WL_CONNECTED) {
      Serial.println("[GreenMind] WiFi lost, reconnecting...");
      WiFi.reconnect();
      int retries = 15;
      while (WiFi.status() != WL_CONNECTED && retries-- > 0) delay(1000);
      if (WiFi.status() != WL_CONNECTED) {
        Serial.println("[GreenMind] WiFi reconnect failed, rebooting");
        ESP.restart();
      }
    }
  }
  ...
  if (now - lastSampleTime >= SAMPLE_INTERVAL_US) {
    ...
    // When batch is full (380 samples = 1 second), send to gateway
...
void sendBatch() {
  ...
  if (code == 200 || code == 201) {
    errorCount = 0;
    Serial.printf("[GreenMind] Streamed %d samples @ %d Hz [OK]\n", BATCH_SIZE, SAMPLE_RATE);
  } else {
    errorCount++;
    Serial.printf("[GreenMind] Stream error: HTTP %d  (errors: %d)\n", code, errorCount);
    if (errorCount > 20) {
      Serial.println("[GreenMind] Too many errors, rebooting");
      ESP.restart();
    }
  }
}
